...
Code signing is not enabled yet. GitHub Actions currently produces unsigned, reproducibility-checked development artifacts. No claim is made that SignPath Foundation or another certificate authority has approved this project.
...
4. two clean builds of each unsigned executable have identical SHA-256 hashes;
...
8. release notes and checksums identify each exact artifact.
...
The reproducibility check applies to each **unsigned** executable. Authenticode adds a signature and usually a trusted timestamp, so a signed file has a different SHA-256 hash. Signed release hashes must therefore be generated after signing and published separately from unsigned build hashes.
...
Get-AuthenticodeSignature .\OkularSessionLauncher.exe | Format-List
Get-FileHash -Algorithm SHA256 .\OkularSessionLauncher.exe
```
